...
public class Fund {
	...
	private final Hashtable<Cusip, Holding> holdings;
	private final Hashtable<Holding, LinkedList<Cusip>> reverseHoldings;
	...
	public String topHoldings(int n) {
		...
		TreeSet<Holding> sortedHoldings = new TreeSet<Holding>(
				reverseHoldings.keySet());
		...
		for (Holding hold : sortedHoldings) {
			...
			for (Cusip cusip : reverseHoldings.get(hold)) {
				fundHoldings.append(numHoldingsAddedToBuffer + ": " + cusip
						+ " " + hold + "\n");
