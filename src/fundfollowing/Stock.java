...
public class Stock implements java.io.Serializable {
	...
	private double getSharesOutstanding(String ticker) {
		...
		try {
			URL url = new URL("http://www.google.com/finance?fstype=bi&q="
					+ ticker);
			try {
		...
		if (mEndEnd.find()) {
			// System.out.println(wholeFile.substring(mStart.start()));
			// System.out.println(wholeFile.substring(mStart.start()+mEnd.end(),mStart.start()+
			// mEnd.end()+mEndEnd.end()+1));
			return new Double(wholeFile.substring(mStart.start() + mEnd.end(),
					mStart.start() + mEnd.end() + mEndEnd.end() + 1));
		}
		return 0;
	}
