...
public class File13F {
	...
	private final File f13F;
	private final Fund fund13F;
	...
	private void initFund() throws IOException {
		...
		// Most common case is where Cusip Number Number and comma is part of
		// number
		// Get rid of commas to identify Cusip Number; Cusip 70,000 is not Cusip
		// 70 000 (, used a space indentifier)
		...
		System.out.println("Number Claimed: " + numClaimedHoldings
				+ " Number of lines: " + numLines);
		System.out.println("Number of matched lines: "
				+ getNumLinesMatched(bestMatch) + " Number unique: "
				+ bestMatch.size());
		System.out.println(matchType);

		// numClaimedHoldings must be less than or equal to the number of lines
		// (consider one line per holding and heading)
		if (numClaimedHoldings > 0
				&& !matchedHoldingsCloseToClaimed(wholeFile,
						getNumLinesMatched(bestMatch))) {
			System.err
					.println("This file's number of matches doesnt match number of claimed holdings: "
							+ f13F.getPath()
							+ "\nNumber of found: "
							+ getNumLinesMatched(bestMatch)
							+ " Number Claimed in File:" + numClaimedHoldings);
	...
	private String removeLinesWithPutOrCall(File f) {
		...
		try {
			arrayString = Grep.grep(f, null,
					"^((?!\\b(CALL|Call|call|PUT|Put|put)\\b).)*$");
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	/*
	 * Functions to verify that number of matches is close to what is expected
	 * given the file
	 */
	...
	public boolean matchedHoldingsCloseToClaimed(String wholeFile, int numFound) {
		...
		if (numClaimedHoldings == 0)
			return true;
		return numFound >= numClaimedHoldings - .25 * numClaimedHoldings; // within
																			// twenty-five
																			// percent
	}
	...
	private void addToHoldingsFromMatches(ArrayList<String> bestMatch)
			throws IOException {
		...
		for (String line : bestMatch) {
			...
			if (matchType.contains("Six Two One"))
				cusipString = cusipString + tempToken.nextToken()
						+ tempToken.nextToken();
			...
			if (tempToken.hasMoreTokens()) {
			...
			} else if (bestMatch.indexOf(line) > 1
					&& (1.0 * numNotEnoughVals)
							/ (1.0 * bestMatch.indexOf(line)) < .25) {
				...
				System.out.println("MatchType: " + matchType + " Line: " + line
						+ " cusip:" + cusip + " value: " + valueTemp
						+ " NewLine: " + newline);
				// Runtime.getRuntime().exec("open "+ f13F.getCanonicalPath());
				sharesTemp = tempToken.nextToken();
			}

			if (matchType.contains("Sole Shares Switched")) { // This means that
																// "Sole" came
																// before the
																// number of
																// shares owned
			...
			if (!stringIsNumber(removeLettersFromEnd(sharesTemp))
					|| !stringIsNumber(valueTemp)) { // error output
			...
			try {
				...
				if (e.toString().contains("multiple")) {
					System.out.println(removeLettersFromEnd(sharesTemp).split(
							"\\.")[0]);
					sharesTemp = removeLettersFromEnd(sharesTemp).split("\\.")[0]
							+ removeLettersFromEnd(sharesTemp).split("\\.")[1]
									.substring(0, 2);
			...
			if (matchType.contains("Additional Value Shares on Separate Lines")) {
				...
				for (int indx = 1; indx < newLineSplit.length; indx++) {
					...
					valueTempDouble += new Double(
							removeLettersFromEnd(temp.nextToken()));
					sharesTempDouble += new Double(
							removeLettersFromEnd(temp.nextToken()));
				}
			}

			if (sharesValueSwitched)
				fund13F.addHoldings(cusip, new Holding(sharesTempDouble,
						valueTempDouble));
			else
				fund13F.addHoldings(cusip, new Holding(valueTempDouble,
						sharesTempDouble));
		}
	...
	public ArrayList<String> getListMatches(String wholeFile) {
		...
		// TODO getNumLinesMatched(bestMatch) should return bestMatch.size if
		// its not Additional Value...
		// if(){
		bestMatchTemp = getListMatchesSharesPriceMarket(wholeFile);
		// System.out.println(bestMatchTemp.size());

		if (bestMatchTemp != null
				&& bestMatchTemp.size() > getNumLinesMatched(bestMatch)) {
			bestMatch = bestMatchTemp;
		...
		if (bestMatchTemp != null
				&& getNumLinesMatched(bestMatchTemp) > getNumLinesMatched(bestMatch)) {
			bestMatch = bestMatchTemp;
		...
		// FOR ONE where cusip and value are meshed call a new one with cusip of
		// length 8 and 9
	...
	// filings/13Fs/2010/QTR1/data/0001104617-10-000002.txt split only 3 columns
	// at a time!
	// filings/13Fs/2010/QTR1/data/0001140361-10-006694.txt has cusips and in
	// other's instance has tickers in cusip col
	// filings/13Fs/2010/QTR1/data/0001144969-10-000006.txt value and shares
	// mashed together so find instance of it where not
	// mashed then get get substring length then do match by substring For each
	// line with a valid cusip
	...
	private static final String SPACE_WON = "[|\t$,= ]+"; // space without
															// newline
	private static final String NUMBER = "[0-9,.]+";
	private static final String SOMETHING_TIL_NEWLINE = "[^\n]+";
	private static final String NEWLINE = "\n";

	// I am allowing the 7,8th characters to be alphanumeric to include
	// non-equities to match number claimed and number found
	private static final String CUSIP = LETTERS_AND_DIGITS + "\\d{2}"
			+ LETTERS_AND_DIGITS + "{3}" + LETTERS_AND_DIGITS + "{1,2}"
			+ "\\d{0,1}";
	private static final String CUSIP_NINE = LETTERS_AND_DIGITS + "\\d{2}"
			+ LETTERS_AND_DIGITS + "{3}" + LETTERS_AND_DIGITS + "{2}"
			+ "\\d{1}";
	private static final String CUSIP_EIGHT = LETTERS_AND_DIGITS + "\\d{2}"
			+ LETTERS_AND_DIGITS + "{3}" + LETTERS_AND_DIGITS + "{2}"
			+ "\\d{0}";
	private static final String MANY_LETTERS = LETTERS + "+";

	private final String sixTwoOneLettersAndDigits = "\\d{3}"
			+ LETTERS_AND_DIGITS + "{3}" + " " + LETTERS_AND_DIGITS + "{2}"
			+ " " + "\\d{1}";

	public ArrayList<String> getListMatchesDefault(String wholeFile) {
		Pattern cusipPattern = Pattern.compile(CUSIP + SPACE + NUMBER + SPACE
				+ NUMBER);
	...
	public ArrayList<String> getListMatchesMashedNine(String wholeFile) {
		// Space_star because there are some where the cusip and value is
		// merged;
		Pattern cusipPattern = Pattern.compile(CUSIP_NINE + SPACE_STAR + NUMBER
				+ SPACE + NUMBER);
		Matcher pm = cusipPattern.matcher(wholeFile);
		return getArrayListOfMatches(pm);
	}
	...
	public ArrayList<String> getListMatchesMashedEight(String wholeFile) {
		...
		Pattern cusipPattern = Pattern.compile(CUSIP_EIGHT + SPACE_STAR
				+ NUMBER + SPACE + NUMBER);
		Pattern cusipPatternTest = Pattern.compile(CUSIP_EIGHT + SPACE_STAR
				+ NUMBER);
	...
	public ArrayList<String> getListMatchesSHSoleShareSwitched(String wholeFile) {
		Pattern cusipPattern = Pattern.compile(CUSIP + SPACE + NUMBER + SPACE
				+ MANY_LETTERS + SPACE + MANY_LETTERS + SPACE + NUMBER);
		Matcher pm = cusipPattern.matcher(wholeFile);
		return getArrayListOfMatches(pm);
	}

	public ArrayList<String> getListMatchesSixTwoOne(String wholeFile) {
		Pattern cusipPattern = Pattern.compile(sixTwoOneLettersAndDigits
	...
	public ArrayList<String> getListMatchesNameCusipSwitched(String wholeFile) {
		Pattern cusipPattern = Pattern.compile(CUSIP + SPACE + NAME + SPACE
				+ NUMBER + SPACE + NUMBER);
		Matcher pm = cusipPattern.matcher(wholeFile);
		return getArrayListOfMatches(pm);
	}

	public ArrayList<String> getListMatchesNameTickerCusipSwitched(
			String wholeFile) {
		Pattern cusipPattern = Pattern.compile(CUSIP + SPACE + NAME + SPACE
				+ NAME + SPACE + NUMBER + SPACE + NUMBER);
		Matcher pm = cusipPattern.matcher(wholeFile);
		return getArrayListOfMatches(pm);
	}

	public ArrayList<String> getListMatchesDefaultAdditionalValueShares(
			String wholeFile) {
		Pattern cusipPattern = Pattern.compile(CUSIP + "(" + SPACE_WON + NUMBER
				+ SPACE_WON + NUMBER + SOMETHING_TIL_NEWLINE + NEWLINE + ")+");
		Matcher pm = cusipPattern.matcher(wholeFile);
		return getArrayListOfMatches(pm);
	}

	public ArrayList<String> getListMatchesSixTwoOneAdditionalValueShares(
			String wholeFile) {
		Pattern cusipPattern = Pattern.compile(sixTwoOneLettersAndDigits + "("
				+ SPACE_WON + NUMBER + SPACE_WON + NUMBER
				+ SOMETHING_TIL_NEWLINE + NEWLINE + ")+");
		Matcher pm = cusipPattern.matcher(wholeFile);
		return getArrayListOfMatches(pm);
	}
	...
	private ArrayList<String> getMatches(String wholeFile,
			Pattern cusipPattern, Pattern cusipPatternTest) {
		Matcher pm = cusipPattern.matcher(wholeFile);
		...
		if (withNumbers.size() > withoutNumbers.size()) // verify that there are
														// more cusips with
														// numbers found than
														// without
			return withNumbers;
